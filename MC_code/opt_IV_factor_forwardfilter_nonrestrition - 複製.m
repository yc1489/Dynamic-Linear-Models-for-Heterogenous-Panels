...
for sig=1:N
 Sigma_u_i(:,sig)=hat_u(:,sig)'*hat_u(:,sig)/T1;   % 1 by 1
 Sigma_eta_i(:,sig)= hat_v_eta_2(:,:,sig)'*hat_v_eta_2(:,:,sig)/T1;
 sigma_etau(:,sig)= hat_v_eta_2(:,:,sig)'*hat_u(:,sig)/T1;
%%%%%%%%%%%%%%%%%%%%%%%%%% 
Sigma_u(:,:,sig) =V_2(:,:,sig)'*V_2(:,:,sig)/T1;  % 1+k by 1+k
 sigma_ue(:,sig)=V_2(:,:,sig)'*hat_u(:,sig)/T1;  %1+k by 1
  sigmaue =sigma_ue(:,sig);
    A1=zeros(1+k,1+k);
    %for op=1:T1
    %A1=A1+first_stage(op,:,sig)*sigmaue'* pinv(H(:,:,sig))*sigmaue*first_stage(op,:,sig)';
    %end
     for op=1:T1
    A1=A1+(Z_1(op,:,sig)*first_stage(:,:,sig))'*sigmaue'* pinv(H(:,:,sig))*sigmaue*(Z_1(op,:,sig)*first_stage(:,:,sig));
    end
    
    A2=zeros(1+k,1+k);
    for op2=1:T1
     A2(:,:)=A2+ (Z_1(op2,:,sig)*first_stage(:,:,sig))'*sigmaue'* pinv(H(:,:,sig))*(Z_1(op2,:,sig)*first_stage(:,:,sig))'*sigmaue'+sigmaue*(Z_1(op2,:,sig)*first_stage(:,:,sig))*pinv(H(:,:,sig))*sigmaue*(Z_1(op2,:,sig)*first_stage(:,:,sig));
    end
 %%%%%%%%%%%%%%%%%%%%%
...
for ome_i_2=1:N
   ...
   H2= H(:,:,ome_i_2);
    ...
    [omega_2, fval_2, exitflag_2, output_2, lambda_2, hessian_2] = MAIV_opt_nores_2(T1,K_2,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_2,Gamma_2, Sigmau_2,sigmaue_2,A1,A2, H2,etai2,d,j2,omega_ini_2,lq_2,uq_2);
...
for ome_i_3=1:N
   ...
   H2= H(:,:,ome_i_3);
    ...
    [omega_3, fval_3, exitflag_3, output_3, lambda_3, hessian_3] = MAIV_opt_nores_3(T1,K_3,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_3,Gamma_3, Sigmau_3,sigmaue_2,A1,A2, H2,etai3,d,j3,omega_ini_3,lq_3,uq_3);
...
for ome_i=1:N
   ...
   lq_4 = [-1*ones(j4,1)];
    ...
    [omega, fval, exitflag, output, lambda, hessian] = MAIV_opt_nores_4(T1,K_4,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_4,Gamma_4, Sigmau_4,sigmaue_2,A1,A2, H2,etai4,d,j4,omega_ini_4,lq_4,uq_4);
...
for ome_i_5=1:N
    ...
    [omega_5, fval_5, exitflag_5, output_5, lambda_5, hessian_5] = MAIV_opt_nores_5(T1,K_5,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_5,Gamma_5, Sigmau_5,sigmaue_2,A1,A2, H2,etai5,d,j5,omega_ini_5,lq_5,uq_5);
...
for ome_i_6=1:N
    ...
    [omega_6, fval_6, exitflag_6, output_6, lambda_6, hessian_6] = MAIV_opt_nores_6(T1,K_6,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_6,Gamma_6, Sigmau_6,sigmaue_2,A1,A2, H2,etai6,d,j6,omega_ini_6,lq_6,uq_6);
...
for ome_i_7=1:N
    ...
    [omega_7, fval_7, exitflag_7, output_7, lambda_7, hessian_7] =MAIV_opt_nores_7(T1,K_7,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_7,Gamma_7, Sigmau_7,sigmaue_2,A1,A2, H2,etai7,d,j7,omega_ini_7,lq_7,uq_7);
