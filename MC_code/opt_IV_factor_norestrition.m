...
fx7=fx(:,TT-T0-5:TT-6); % m by T0; F_(x,-5)
fx8=fx(:,TT-T0-6:TT-7); % m by T0; F_(x,-5)
...
MF_x7=eye(T0)-fx7'*((fx7*fx7')^(-1))*fx7;   %MF_x,-5 T0 by T0
MF_x8=eye(T0)-fx8'*((fx8*fx8')^(-1))*fx8;   %MF_x,-5 T0 by T0
...
Z_it_6=zeros(N,T0,1*k);           % N by T  by 5k
Z_it_6(:,:,1)=(MF_x7*x_NT1_6')';  % N by T  
Z_it_6(:,:,2)=(MF_x7*x_NT2_6')';  % N by T 


Z_it_7=zeros(N,T0,1*k);           % N by T  by 5k
Z_it_7(:,:,1)=(MF_x8*x_NT1_7')';  % N by T  
Z_it_7(:,:,2)=(MF_x8*x_NT2_7')';  % N by T 




Z_it_M=zeros(N,T0,(j+1)*k);           % N by T  by 5k
...
P_i_7=zeros(T0,T0,N);
 ...
 P_i_M6=zeros(T0,T0,N);
 P_i_M7=zeros(T0,T0,N);
for p=1:N
...
Sigma_u=zeros(1+k,1+k,N);
sigma_ue=zeros(1+k,N);
...
V_eta_M6_1=zeros(T0,N);   %
V_eta_M6_2=zeros(T0,N);  %
V_eta_M6_3=zeros(T0,N);   %
V_eta_M6_4=zeros(T0,N);  %
V_eta_M6_5=zeros(T0,N);  %
V_eta_M6_6=zeros(T0,N);  %

V_eta_M7_1=zeros(T0,N);   %
V_eta_M7_2=zeros(T0,N);  %
V_eta_M7_3=zeros(T0,N);   %
V_eta_M7_4=zeros(T0,N);  %
V_eta_M7_5=zeros(T0,N);  %
V_eta_M7_6=zeros(T0,N);  %
V_eta_M7_7=zeros(T0,N);  %
...
%%%%%%%%%%%%%%%%%%%%%%%%%% 
Sigma_u(:,:,sig) =V_2(:,:,sig)'*V_2(:,:,sig)/T0;  % 1+k by 1+k
 sigma_ue(:,sig)=V_2(:,:,sig)'*hat_u(:,sig)/T0;  %1+k by 1
  sigmaue =sigma_ue(:,sig);
    A1=zeros(1+k,1+k);
    for op=1:T0
    A1=A1+H1(op,:,sig)'*sigmaue'* pinv(H(:,:,sig))*sigmaue*H1(op,:,sig);
    end
    
    A2=zeros(1+k,1+k);
    for op2=1:T0
     A2(:,:)=A2+ H1(op2,:,sig)'*sigmaue'* pinv(H(:,:,sig))*H1(op2,:,sig)'*sigmaue'+sigmaue*H1(op2,:,sig)*pinv(H(:,:,sig))*sigmaue*H1(op2,:,sig);
    end
 %%%%%%%%%%%%%%%%%%%%%
...
for ome_i_2=1:N
    ...
    etai2=etai(:,ome_i_2);
    sigmaue_2 =sigma_ue(:,ome_i_2);
    omega_ini_2 = rand(j2,1);
    lq_2 = [-1*ones(j2,1)];
    uq_2 = [1*ones(j2,1)];
   H2= H(:,:,ome_i_2);
    Sigmau_2= Sigma_u(:,:,ome_i_2);
    sigma_etau_i=sigma_etau(:,ome_i_2); 
    Sigma_u_i_i=Sigma_u_i(:,ome_i_2);
    Sigma_eta_i_i=Sigma_eta_i(:,ome_i_2);
    hat_U_i_2=hat_U2(:,:,ome_i_2);    
    [omega_2, fval_2, exitflag_2, output_2, lambda_2, hessian_2] = MAIV_opt_nores_2(T0,K_2,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_2,Gamma_2, Sigmau_2,sigmaue_2,A1,A2, H2,etai2,d,j2,omega_ini_2,lq_2,uq_2);
...
for ome_i_3=1:N
    ...
    etai3=etai(:,ome_i_3);  %%%%
        sigmaue_2 =sigma_ue(:,ome_i_3);
    omega_ini_3 = rand(j3,1);
    lq_3 = [-1*ones(j3,1)];
    uq_3 = [1*ones(j3,1)];
   H2= H(:,:,ome_i_3);
    Sigmau_3= Sigma_u(:,:,ome_i_3);
    sigma_etau_i=sigma_etau(:,ome_i_3); 
    Sigma_u_i_i=Sigma_u_i(:,ome_i_3);
    Sigma_eta_i_i=Sigma_eta_i(:,ome_i_3);
    hat_U_i_3=hat_U3(:,:,ome_i_3);    
    [omega_3, fval_3, exitflag_3, output_3, lambda_3, hessian_3] = MAIV_opt_nores_3(T0,K_3,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_3,Gamma_3, Sigmau_3,sigmaue_2,A1,A2, H2,etai3,d,j3,omega_ini_3,lq_3,uq_3);
...
for ome_i=1:N
    etai4=etai(:,ome_i);
    sigmaue_2 =sigma_ue(:,ome_i);
    omega_ini_4 = rand(j4,1);
   lq_4 = [-1*ones(j4,1)];
    uq_4 = [1*ones(j4,1)];
    H2= H(:,:,ome_i);
    Sigmau_4= Sigma_u(:,:,ome_i);
    sigma_etau_i=sigma_etau(:,ome_i); 
    Sigma_u_i_i=Sigma_u_i(:,ome_i);
    Sigma_eta_i_i=Sigma_eta_i(:,ome_i);
    hat_U_i_4=hat_U4(:,:,ome_i);    
    [omega, fval, exitflag, output, lambda, hessian] = MAIV_opt_nores_4(T0,K_4,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_4,Gamma_4, Sigmau_4,sigmaue_2,A1,A2, H2,etai4,d,j4,omega_ini_4,lq_4,uq_4);
...
for ome_i_5=1:N
    etai5=etai(:,ome_i_5);
    sigmaue_2 =sigma_ue(:,ome_i_5);
    omega_ini_5 = rand(j5,1);
    lq_5 = [-1*ones(j5,1)];
    uq_5 = [1*ones(j5,1)];
    H2= H(:,:,ome_i_5);
    Sigmau_5= Sigma_u(:,:,ome_i_5);
    sigma_etau_i=sigma_etau(:,ome_i_5); 
    Sigma_u_i_i=Sigma_u_i(:,ome_i_5);
    Sigma_eta_i_i=Sigma_eta_i(:,ome_i_5);
    hat_U_i_5=hat_U5(:,:,ome_i_5);    
    [omega_5, fval_5, exitflag_5, output_5, lambda_5, hessian_5] = MAIV_opt_nores_5(T0,K_5,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_5,Gamma_5, Sigmau_5,sigmaue_2,A1,A2, H2,etai5,d,j5,omega_ini_5,lq_5,uq_5);
...
for ome_i_6=1:N
    etai6=etai(:,ome_i_6);
    sigmaue_2 =sigma_ue(:,ome_i_6);
    omega_ini_6 = rand(j6,1);
    lq_6 = [-1*ones(j6,1)];
    uq_6 = [1*ones(j6,1)];
    H2= H(:,:,ome_i_6);
    Sigmau_6= Sigma_u(:,:,ome_i_6);
    sigma_etau_i=sigma_etau(:,ome_i_6); 
    Sigma_u_i_i=Sigma_u_i(:,ome_i_6);
    Sigma_eta_i_i=Sigma_eta_i(:,ome_i_6);
    hat_U_i_6=hat_U6(:,:,ome_i_6);    
    [omega_6, fval_6, exitflag_6, output_6, lambda_6, hessian_6] = MAIV_opt_nores_6(T0,K_6,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_6,Gamma_6, Sigmau_6,sigmaue_2,A1,A2, H2,etai6,d,j6,omega_ini_6,lq_6,uq_6);
...
for ome_i_7=1:N
    etai7=etai(:,ome_i_7);
    sigmaue_2 =sigma_ue(:,ome_i_7);
    omega_ini_7 = rand(j7,1);
    lq_7 = [-1*ones(j7,1)];
    uq_7 = [1*ones(j7,1)];
    H2= H(:,:,ome_i_7);
    Sigmau_7= Sigma_u(:,:,ome_i_7);
    sigma_etau_i=sigma_etau(:,ome_i_7); 
    Sigma_u_i_i=Sigma_u_i(:,ome_i_7);
    Sigma_eta_i_i=Sigma_eta_i(:,ome_i_7);
    hat_U_i_7=hat_U7(:,:,ome_i_7);    
    [omega_7, fval_7, exitflag_7, output_7, lambda_7, hessian_7] =MAIV_opt_nores_7(T0,K_7,sigma_etau_i,Sigma_u_i_i,Sigma_eta_i_i, hat_U_i_7,Gamma_7, Sigmau_7,sigmaue_2,A1,A2, H2,etai7,d,j7,omega_ini_7,lq_7,uq_7);
